...
///
/// This intentionally describes the transcription output, not NeverType's UI
...
/// Filler tokens that are not lexical words in any language NeverType's models can
...
mod tests {
    ...
    fn test_apply_custom_words_handles_unicode_punctuation() {
        let text = "「Handee。」";
        let custom_words = vec!["NeverType".to_string()];
        let result = apply_custom_words(text, &custom_words, 0.5);
        assert_eq!(result, "「NeverType。」");
    }
