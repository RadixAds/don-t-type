...
fn build_headers(provider: &PostProcessProvider, api_key: &str) -> Result<HeaderMap, String> {
    ...
    headers.insert(
        REFERER,
        HeaderValue::from_static("https://nevertype.app"),
    );
    headers.insert(
        USER_AGENT,
        HeaderValue::from_static("NeverType/1.0 (+https://nevertype.app)"),
    );
    headers.insert("X-Title", HeaderValue::from_static("NeverType"));
